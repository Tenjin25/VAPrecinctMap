...
| **Selected locality panel** | Premium **Virginia Locality Focus** panel (county + independent city aware). Narrative order: summary hero (winner, margin, total votes, shift) → one-line at-a-glance → trend/trajectory → population signal + census check → demographics → precinct/non-geographic context (when available) → what to watch. Includes a compact “More detail” collapsible for deeper context (why it votes this way, locality profile, vote breakdown). |
