...
| **Hover tooltip** | Condensed broadcast hover card for **counties, independent cities, precincts, and districts**. Shows a one-line margin call + tier, plus (when available) **Flip** context (line + chip) and an NC-style delta block (Shift mode only) with party-colored raw vote changes vs the prior comparable election, plus optional population-change lines. `Pin` freezes the tooltip (and reveals `Details` + `Close`); `Close` dismisses and won’t immediately re-open until you move the pointer (prevents hover flicker). Mobile docks the tooltip above the legend/results so it stays readable, and tapping a county pins the tooltip. County overlay layers (for example, Smart Insights glow/outline) drive the same hover tooltip behavior. |
